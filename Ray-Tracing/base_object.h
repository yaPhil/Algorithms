...
public:
    ...
    Triangle(Color color, Vector a, Vector b, Vector c) : SolidObject(color), a_(a), b_(b), c_(c) {
        double xu = std::max(std::max(a_.getX(), b_.getX()), c_.getX());
        double xd = std::min(std::min(a_.getX(), b_.getX()), c_.getX());
        double yu = std::max(std::max(a_.getY(), b_.getY()), c_.getY());
        double yd = std::min(std::min(a_.getY(), b_.getY()), c_.getY());
        double zu = std::max(std::max(a_.getZ(), b_.getZ()), c_.getZ());
        double zd = std::min(std::min(a_.getZ(), b_.getZ()), c_.getZ());
...
public:
    ...
    Sphere(Color color, Vector c, double r) : SolidObject(color), center_(c), rad_(r) {
        double xu = center_.getX() + rad_, xd = center_.getX() - rad_;
        double yu = center_.getY() + rad_, yd = center_.getY() - rad_;
        double zu = center_.getZ() + rad_, zd = center_.getZ() - rad_;
        setBox(BoundBox(xu, xd, yu, yd, zu, zd));
    }
...
public:
    ...
    Quadrangle(Color color, Vector a, Vector b, Vector c, Vector d) : SolidObject(color), a_(a), b_(b), c_(c), d_(d) {
        double xu = std::max(std::max(a_.getX(), b_.getX()), std::max(c_.getX(), d_.getX()));
        double xd = std::min(std::min(a_.getX(), b_.getX()), std::min(c_.getX(), d_.getX()));
        double yu = std::max(std::max(a_.getY(), b_.getY()), std::max(c_.getY(), d_.getY()));
        double yd = std::min(std::min(a_.getY(), b_.getY()), std::min(c_.getY(), d_.getY()));
        double zu = std::max(std::max(a_.getZ(), b_.getZ()), std::max(c_.getZ(), d_.getZ()));
        double zd = std::min(std::min(a_.getZ(), b_.getZ()), std::min(c_.getZ(), d_.getZ()));
        setBox(BoundBox(xu, xd, yu, yd, zu, zd));
    }
    Vector getNorm(Vector p) override;
    Intersect intersectRay(Ray ray) override;
